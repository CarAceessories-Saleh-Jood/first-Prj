...
public class ManageProductsT {
	...
	@Given("that the admin is logged in and he wants to see the products")
	public void thatTheAdminIsLoggedInAndHeWantsToSeeTheProducts() {
	    obj.setIsLogged(true);

	}

	@Then("the product will not be found or deleted")
	public void theProductWillNotBeFoundOrDeleted() {
		int productsBeforedelete = obj1.getProductList().size();
		obj1.deleteProduct(productName);
		
	    int productsAfterdelete = obj1.getProductList().size();
	    assertEquals(productsBeforedelete, productsAfterdelete );    
	}
	@Then("the products will be printed")
	public void theProductsWillBePrinted() {
